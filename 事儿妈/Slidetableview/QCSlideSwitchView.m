...
static const CGFloat kWidthOfButtonMargin = 23.0f;
...
- (void)initValues
{
    ...
    //_topScrollView.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageNamed:@"ScrollViewBack"]];
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset+10,0,
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin+10, 0, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }
